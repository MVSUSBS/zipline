...
class AssetFinderTestCase(TestCase):
    ...
    def test_compute_lifetimes(self, env=None):
        ...
        for dates in all_subindices(all_dates):
            ...
            # Doing the dumbest-simplest iterative computation possible here.
            # Don't write numpy code like this if it needs to be performant...
            ...
            for i, date in enumerate(dates):
                ...
                for j, start, end in it:
                    if start <= date <= end:
                        ...
                        nonempty_columns.add(j)

            # Filter out columns with all-empty columns.
            nonempty_columns = list(sorted(nonempty_columns))
            expected_result = pd.DataFrame(
                data=expected_mask[:, nonempty_columns],
                index=dates,
